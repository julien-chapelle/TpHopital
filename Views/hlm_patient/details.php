    ...
    <div class="card text-center m-5 shadow cardForm">
        <?php foreach ($detailPatient as $value) { ?>
            ...
            <div class="card-body">
                ...
                <p class="card-text">DATE DE NAISSANCE : <?= utf8_encode(strftime('%d %B %Y', strtotime($value['birthdate']))) ?></p>
        ...
        <div class="card-footer border-0">
            ...
            <ul class="list-group">
                <?php foreach ($detailAppointment as $value) { ?>
                    <li class="list-group-item"><?= utf8_encode(strftime('%A %d %B %Y', strtotime($value['dateHour']))) . ' à ' . strftime('%Hh%M', strtotime($value['dateHour'])) ?></li>
                <?php }; ?>
            </ul>
        </div>
    </div>
